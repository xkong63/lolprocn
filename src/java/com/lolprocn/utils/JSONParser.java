...
public class JSONParser {
    ...
    public PlayerHistory getPlayerHistory(long summonerId) throws JSONException {
        PlayerHistory playerHistory = new PlayerHistory();
        List<MatchSummary> matchList = new ArrayList<>();
        String response = new String();
        try {
            response = Connection.sendGet("https://na.api.pvp.net/api/lol/na/v2.2/matchhistory/" + summonerId + "?api_key=81a9f77d-eb19-4581-a58e-afc46b10ed0b");
    ...
    public MatchSummary populateMatchSummary(JSONObject match) throws JSONException {
        ...
        for (int i = 0; i < participantIdentities.length(); i++) {
            ParticipantIdentity identity = populateParticipantIdentity(participantIdentities.getJSONObject(i));
            participantIdentitiesList.add(identity.getParticipantId(), identity);
        }

        JSONArray participants = match.getJSONArray("participants");
        List<Participant> participantsList = new ArrayList<Participant>();
        for (int i = 0; i < participants.length(); i++) {
            Participant p = populateParticipant(participants.getJSONObject(i));
            participantsList.add(p.getParticipantId(), p);
        }
        populateOtherParticipant(match.getLong("matchId"), participantIdentitiesList, participantsList);
    ...
    public void populateOtherParticipant(long matchId, List<ParticipantIdentity> participantIdentitiesList, List<Participant> participantsList) throws JSONException {
        String response = new String();
        try {
            response = Connection.sendGet("https://na.api.pvp.net/api/lol/na/v2.2/match/" + matchId + "?api_key=81a9f77d-eb19-4581-a58e-afc46b10ed0b");
            JSONObject jobj = new JSONObject(response);
            JSONArray participantIdentities = jobj.getJSONArray("participantIdentities");
            for (int i = 0; i < participantIdentities.length(); i++) {
                ParticipantIdentity identity = populateParticipantIdentity(participantIdentities.getJSONObject(i));
                participantIdentitiesList.add(identity.getParticipantId(), identity);
            }

            JSONArray participants = jobj.getJSONArray("participants");
            for (int i = 0; i < participants.length(); i++) {
                Participant p = populateParticipant(participants.getJSONObject(i));
                participantsList.add(p.getParticipantId(), p);
            }
    ...
    public ParticipantStats populateParticipantStats(JSONObject participantStats) throws JSONException {
        ...
        stats.setWinner(participantStats.getBoolean("winner"));
        return stats;
    }
    ...
    public SummonerDto populateSummonerDto(String name) throws JSONException, IOException {
        ...
        String response = Connection.sendGet("https://na.api.pvp.net/api/lol/na/v1.4/summoner/by-name/" + name.replaceAll("\\W+", "") + "?api_key=018a4d88-bbb4-4578-aec5-8b3bf049bb12");
        JSONObject jObject0 = new JSONObject(response);

        JSONObject jObject = jObject0.getJSONObject(name.toLowerCase().replaceAll("\\W+", ""));
    ...
    public ChampionDto populateChampionDto(JSONObject jObject) throws JSONException, IOException {
        ...
        championDto.setName(JSONParser.convertFromIdToName(jObject.getLong("id")));
        System.out.println(championDto.getName());
        return championDto;
    }

    public static String convertFromIdToName(Long id) throws IOException, JSONException {
